...
# https://github.com/owncloud/enterprise/issues/1095
...
# win conflict with https://www.rpmfind.net/linux/RPM/epel/7/x86_64/o/owncloud-httpd-8.0.10-1.el7.noarch.html
# https://github.com/owncloud/enterprise/issues/1076
Obsoletes:    %{name}-httpd
License:      AGPL-3.0 and MIT
Group:        Development/Libraries/PHP
...
if [ $1 -eq 1 ]; then
    ...
    echo "%{name}-server: Upgrade installed"
fi
...
if [ 0$1 -eq 0 ]; then
  ...
  echo "You just uninstalled %{name} . Please also check these: "
  rpm -qa 'owncloud-*'
fi
