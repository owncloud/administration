...
%define server_min_version %{base_version}
%else
%define server_min_version %{base_version}~%{prerelease}
# % define server_min_version %{base_version}
%endif
...
Source6:        https://doc.owncloud.org/server/8.0/ownCloudServerAdminManual.pdf
...
BuildRequires:	owncloud >= %{server_min_version}
...
Requires:	owncloud >= %{server_min_version}
Requires:	%{name}-theme
...
#################
# Not ready for 8.0.1
# Requires:	%{name}-app-search_elastic
#################
...
Group:          Productivity/Networking/Web/Utilities		\
Requires:	owncloud-enterprise-server >= %{server_min_version} 	\
%{?2:%{2}} 					\
%description app-%{1} 				\
...
%if %{fhs}
mv $idir/config/* $RPM_BUILD_ROOT/%{oc_config_dir}
...
#               files_ldap_home		Requires:php5-ldap
