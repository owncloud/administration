...
class OCC():
    ...
    with open(file) as cfg:
      for line in cfg:
        for m in re.finditer('(["\'])(.*?)\\1\s*=>\s*((["\'])(.*?)\\1|([\w]+))\s*,', line):
          ...
          self._config[m.group(2)] = m.group(5) or m.group(6)
  ...
  def db_cursor(self):
    if self._db is None:
      raise ValueError("not connected. Call db_connect() first.")
    dbc = self._db.cursor()
  ...
  def fetch_oc_mounts(self, dbc):
    ...
    """
    self._oc_mounts = self.db_fetch_dict(dbc, 'SELECT m.*, c.path as root_path FROM '+self.oc_+'mounts m LEFT JOIN '+self.oc_+'filecache c ON (m.root_id = c.fileid)', 'mount_point')
  ...
  def find_mountpoint(self, path):
    ...
    This implicitly calls fetch_oc_mounts() if not previously called to
